...
- Menggunakan `dryoc::dryocbox` (NaCl box: X25519 + XSalsa20-Poly1305)
...
- Menggunakan `dryoc::dryocsecretbox` (NaCl secretbox)
...
#### [NEW] `lib/src/core/database/database.dart`
- Drift database class, table definitions, migration strategy
- Full schema dari blueprint Bagian 6 (7 tabel: identity, contacts, conversations, messages, groups, group_members, message_queue)

#### [NEW] `lib/src/core/database/tables/identity_table.dart`
- Drift table definition untuk `identity`

#### [NEW] `lib/src/core/database/tables/contacts_table.dart`
- Drift table definition untuk `contacts`

#### [NEW] `lib/src/core/database/tables/conversations_table.dart`
- Drift table definition untuk `conversations`

#### [NEW] `lib/src/core/database/tables/messages_table.dart`
- Drift table definition untuk `messages`

#### [NEW] `lib/src/core/database/tables/groups_table.dart`
- Drift table definition untuk `groups` + `group_members`

#### [NEW] `lib/src/core/database/tables/message_queue_table.dart`
- Drift table definition untuk `message_queue`

#### [NEW] `lib/src/core/database/dao/identity_dao.dart`
- Type-safe CRUD operations untuk tabel `identity` (Drift DAO)

#### [NEW] `lib/src/core/database/dao/contacts_dao.dart`
- Type-safe CRUD operations untuk tabel `contacts` (Drift DAO)

#### [NEW] `lib/src/core/database/dao/conversations_dao.dart`
- Type-safe CRUD operations untuk tabel `conversations` (Drift DAO)

#### [NEW] `lib/src/core/database/dao/messages_dao.dart`
- Type-safe CRUD operations untuk tabel `messages` (Drift DAO)
- Pagination support

#### [NEW] `lib/src/core/database/dao/groups_dao.dart`
- Type-safe CRUD operations untuk tabel `groups` + `group_members` (Drift DAO)

#### [NEW] `lib/src/core/database/dao/message_queue_dao.dart`
- Type-safe CRUD operations untuk tabel `message_queue` (Drift DAO)
...
**Verification Fase 1:**
- Rust unit tests: `cargo test` — semua crypto roundtrip pass (dryoc)
- Rust integration test: 2 node lokal bisa connect via mDNS
- Flutter unit tests: semua Drift DAO CRUD operations
...
- `saveContactFromQR(data)` — parse, validate, simpan ke Drift database
...
- `receiveDM(envelope)` — call Rust decrypt, simpan ke Drift database
...
| Flutter DAO | `flutter test` | 100% — all Drift DAO operations |
